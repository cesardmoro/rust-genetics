...
function createElement(parent, type, text, classList, attrs){
...
function displayCrop(crop){
    ...
    let cropNumber = crops.indexOf(crop);
    for(let i = 0; i < 6; i++){
        let c = crop.charAt(i);
        createElement(list, 'span', c, ( (c == 'X' || c == 'W') ? 'bad' : 'good' ) + ' gene row-'+cropNumber  )
    }
    createElement(list, 'span', '<img src="assets/delete-24px.svg"></img>', 'remove row-'+cropNumber, {'onclick':'removeCrop('+cropNumber+')'});
    createElement(list, 'br', '', 'row-'+cropNumber);
}
function renderCrops(crops){
    for (let key in crops){
        displayCrop(crops[key]);
    }
}
function removeCrop(cropIndex){
